...
function rackMiners() {
  ...
  var macAddress = targetSheet.getRange(targetCell1).getValue();
  var snNum = targetSheet.getRange(targetCell2).getValue();

  // Insert the value into the specified cell, running state, remove # sign
  targetSheet.getRange(targetCell1).setValue(macAddress.replace(/[#!]/g, '')).setBackground(null)
  targetSheet.getRange(targetCell2).setValue(snNum.replace(/[#!]/g, '')).setBackground(null)
